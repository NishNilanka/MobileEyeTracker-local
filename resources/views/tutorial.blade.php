...
<body>
	...
	<div class="modal fade" id="ConfModal" data-keyboard="false" data-backdrop="static">
		<div class="modal-dialog modal-dialog-scrollable">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button id="pingTestbutton" onclick="pingTest();" type="button" class="btn btn-lg btn-success">Ping Test</button>
					<button id="btnStart" onclick="startRecorder();" type="button" data-bs-dismiss="modal" class="btn btn-lg btn-success" disabled>Begin</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div id="button-container" class="button-container" style="display: none">
		...
		<form id="luxValueForm" action="{{ route('luxValue.store') }}" method="POST">
			@csrf
			<div class="col-md-4 col-centered">
				<label for="luxValue">Enter lux value:</label> <br>
				<input type="number" class="form-control" id="luxValue" name="luxValue" min="0">
			</div>
			<br>
			<div class="col-md-4 col-centered">
		...
		<div class="col-md-4 col-centered">
			<button class="button proceed-button" disabled style="display: none;">Proceed</button>
		</div>
		<br>
		<div class="col-md-4 col-centered">
...
<script>
	...
	const luxValueInput = document.getElementById('luxValue');
	const submitButton = document.getElementById('submitBtn');
	luxValueInput.addEventListener('input', enableButton);
	// Function to enable/disable the submit button based on input
	function enableButton() {
		if (luxValueInput.value.trim() !== '') {
	...
	luxValueForm.addEventListener('submit', async (event) => {
		event.preventDefault(); // Prevent the default form submission

		// Get the lux value from the input field
		const luxValue = document.getElementById('luxValue').value;

		// Create a FormData object and append the data
		const luxData = new FormData();
		luxData.append('_token', '{{ csrf_token() }}');
		luxData.append("videoID", localStorage.getItem("vidCurrentPlayingNumber"));
		luxData.append('luxValue', luxValue);

		// Send a POST request to the server
		const response = await fetch('/luxValue', {
			method: 'POST',
			body: luxData,
		});

		// Check the response status and enable the "proceed" button
		if (response.ok) {
			...
			console.error('Failed to submit the lux value.');
		}
	});
	...
	async function displayCalibrationPoint() {
		...
		if (currentIndex < calibrationPoints.length) {
			...
			await startCalibration(currentIndex);
		} else {
	...
	function sendPingTestResults(averageRTT, averageTimeDifference, stdRTT) {
		fetch('https://192.168.50.183:8100/send_ping_results', {
				method: 'POST',
				headers: {
				...
				body: JSON.stringify({
					...
					videoID: localStorage.getItem("vidCurrentPlayingNumber"),
					avg_rtt: averageRTT,
					avg_time_diff: averageTimeDifference,
					std_rtt: stdRTT
				})
			})
	...
	function sendSequentialRequest(index) {
		if (index >= numRequests) {
			...
			if (completedRequests > 0) {
				const averageRTT = totalRTT / completedRequests;
				const averageTimeDifference = totalTimeDifference / completedRequests;
				const stdRTT = calculateStdDev(rttValues);
				console.log(`Average RTT: ${averageRTT.toFixed(2)}ms`);
				console.log(`Average Time Difference: ${averageTimeDifference.toFixed(2)}ms`);
				console.log(`Standard Deviation of RTT: ${stdRTT.toFixed(2)}ms`);

				sendPingTestResults(averageRTT, averageTimeDifference, stdRTT);
				document.getElementById("btnStart").removeAttribute("disabled");
				document.getElementById("pingTestbutton").style.display = 'None';
			}
			return;
		}
		let currentTimerequest = getCurrentTimeWithMilliseconds();
		const startTime = performance.now();
		let currentTimeresponse;
		...
		fetch(fetchURL)
			...
			.then(data => {
				...
				currentTimeresponse = getCurrentTimeWithMilliseconds();
				rttValues.push(roundTripTime);


				// Accumulate RTT and time difference values
				totalRTT += roundTripTime;


				// Handle the response from the backend
				//console.log(`Request ${index + 1} - Phone time (before request) - ${currentTimerequest}`);
				//console.log(`Request ${index + 1} - Server Time - ${data.current_time}`);
				//console.log(`Request ${index + 1} - RTT - ${roundTripTime.toFixed(2)}ms`);

				// Define currentTimeresponse here

				//console.log(`Request ${index + 1} - Phone time (after response) - ${currentTimeresponse}`);

				// Correctly calculate time difference with milliseconds
				const timediff = getTimeDifference(currentTimerequest, data.current_time, roundTripTime);

				totalTimeDifference += timediff;

				//console.log(`Request ${index + 1} - Time Difference - ${timediff.toFixed(2)}ms}`);
	...
	function getTimeDifference(PhoneTime, ServerTime, rtt) {
		// Parse the timestamp strings into Date objects
		const date1 = new Date(`1970-01-01T${PhoneTime}Z`);
		const date2 = new Date(`1970-01-01T${ServerTime}Z`);

		// Calculate the time difference in milliseconds
		const differenceMillis = date1 - date2 + rtt / 2; // Add half of RTT
	...
	function stopEyeTracker() {
		fetch('https://192.168.50.183:8100/stop_eyetracker', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
	...
	function startEyeTracker() {
		fetch('https://192.168.50.183:8100/start_eyetracker', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
	...
	async function startCalibration(index) {

		console.log(index);

		proceedButton.setAttribute('disabled', 'disabled');
		document.getElementById("luxValue").value = "";
		submitButton.setAttribute('disabled', 'disabled');
		...
		try {
			...
			if (data.calibration_status === 'failure') {
				console.log('Calibration failed. You need to recalibrate from the beginning.');
				displayNotification('Calibration failed. You need to recalibrate from the beginning.', false);
				currentIndex = 100;
				displayCalibrationPoint();
			} else if (data.calibration_status === 'success') {
	...
	function displayNotification(message, isSuccess) {
		...
		// You can hide the notification after a certain time if desired.
		setTimeout(() => {
			...
			function(mediaStreamObj) {
				...
				mediaRecorder.onstop = (ev) => {
					...
					fetch('camerafeatures', {
						...
						.then(response => {
							console.log('Camera data upload success')
						})
						.catch(error => {
							console.log('Camera data upload error');
						})
					...
					fetch('deviceorientation', {
						...
						.then(response => {
							console.log('device orientation upload success')
						})
						.catch(error => {
							console.log('device orientation upload error');
						})
	...
	function showUploadingGif() {
		...
		document.getElementById("webcam").style.display = "none";
		document.getElementById("pingTestbutton").style.display = 'block';
		//document.getElementById("userInfo").style.display = "none";
